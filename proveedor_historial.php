...
<body class="#eceff1 blue-grey lighten-4">
    ...
    <div class="row escritorio" >
      <div class="col l12 m12 s12">
        <h3 style="font-family: Helvetica Neue, Helvetica, Arial, sans-serif;">Catálogo de proveedores</h3>
      </div>
      <div class="col l12 m12 s12">
      ...
      <div class="col l12 m12 s12">
        ...
        <tbody>
            <?php 
                ...
                $stmt=$db_con->prepare("SELECT t.nombre as nombre1, t.rfc as rfc1, p.contrato as contrato1, p.fecha_contrato as fecha_contrato1, t.titular as titular1, t.latitud as latitud1, t.longitud as longitud1 from proveedor p INNER JOIN taller t on p.id_taller = t.id");
                $stmt->execute();
                while($fila=$stmt->fetch(PDO::FETCH_ASSOC)) 
                { 
                   echo '<tr><td>'.$fila["nombre1"].'</td>';
                   echo '<td>'.$fila["rfc1"].'</td>';
                   echo '<td>'.$fila["contrato1"].'</td>';
                   echo '<td>'.$fila["fecha_contrato1"].'</td>';
                   echo '<td>'.$fila["titular1"].'</td>';
                   echo '<td>'.$fila["latitud1"].'</td>';
                   echo '<td>'.$fila["longitud1"].'</td></tr>';
                }             
            ?>
        </tbody>
      </table>
