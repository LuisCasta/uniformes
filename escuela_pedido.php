...
<head>
   ...
   <script src="crud_escuela_pedido/escuela_pedido.js" type="text/javascript"></script>
  ...
  <nav class="#01579b light-blue darken-4">
    ...
    <div class="row escritorio" >
      ...
      <div class="col l12 m12 s12">
        <h3 id="titulo" style="font-family: Helvetica Neue, Helvetica, Arial, sans-serif;">Pedidos</h3>
      </div>
      
      <div class="col l4 m12 s12">
        <select class="browser-default red white-text"id="selector_escuela" style="border-radius: 20px">
        <option value="seleccione" selected>Seleccione una matricula</option>       
        <?php 
                require_once 'db_config.php'; 
                $stmt=$db_con->prepare("SELECT matricula,nombre FROM escuela order by matricula ASC");
                $stmt->execute();
      ...
      <div class="col l4 m12 s12">
        <select class="browser-default red white-text"id="selector_tallerp" style="border-radius: 20px">
        <option value="seleccione" selected>Seleccione un taller/proveedor</option>       
        <?php 
            require_once 'db_config.php'; 
            $stmt=$db_con->prepare("SELECT t.nombre as nombreproveedor, p.id as id1 FROM proveedor p INNER JOIN taller t on p.id_taller = t.id");
            $stmt->execute();
            while($fila=$stmt->fetch(PDO::FETCH_ASSOC)) 
            { 
               echo '<option value="'.$fila["id1"].'">'.$fila["nombreproveedor"].'</option>';
            }             
      ?>
        </select><br><br>
      </div>
      <div class="col l4 m12 s12" style="margin-top: 30px;">
